...
{viewerjumpto "Syntax" "uhtred_postestimation##syntax"}{...}
{viewerjumpto "Description" "uhtred_postestimation##description"}{...}
{viewerjumpto "Options" "uhtred_postestimation##options"}{...}
{viewerjumpto "Remarks" "uhtred_postestimation##remarks"}{...}
{viewerjumpto "Examples" "uhtred_postestimation##examples"}{...}

{marker syntax}{...}
{title:Syntax for predict}

{pstd}
Syntax for predictions following a {helpb uhtred:uhtred} model

{p 8 16 2}
{cmd:predict}
{it:newvarname}
{ifin} [{cmd:,}
{it:{help uhtred_postestimation##statistic:statistic}}
{it:{help uhtred_postestimation##opts_table:options}}]
...
{synopt :{opth causes(numlist)}}specify which {cmd:uhtred} submodels contribute to the {it:statistic}{p_end}
...
This entry concerns use of {cmd:predict} after {cmd:uhtred}.

{pstd}
{cmd:predict} after {cmd:uhtred} creates new variables containing
...
In a competing risks {cmd:uhtred} model, all survival models in the fitted {cmd:uhtred} model are assumed to 
...
models have been specified in your {cmd:uhtred} model, then it will assume all of them are cause-specific competing risks models, 
...
If multple survival models are specified in the {cmd:uhtred} model then by default all are assumed to be cause-specific 
...
to {cmd:timelost}. If multiple survival models are specified in the {cmd:uhtred} model then by default all are 
...
the variance components are listed in the output of {cmd:uhtred}.


{dlgtab:Options}

{phang}
{cmd:causes(numlist)} is for use when calculating predictions from a competing risks {cmd:uhtred} model. By default, 
{cmd:cif}, {cmd:rmst}, {cmd:timelost} and {cmd:totaltimelost} assume that all survival models included in the {cmd:uhtred} 
...
models (indexed using the order they appear in your {cmd:uhtred} model, e.g. {cmd:causes(1 2)}), by using the {cmd:causes()} 
option.
...
For a {cmd:uhtred} model where a {cmd:timevar()} was specified, then the default will use the original 
...
{phang2}{cmd:. uhtred (_t hormon, family(rp, df(3) failure(_d)))}{p_end}
