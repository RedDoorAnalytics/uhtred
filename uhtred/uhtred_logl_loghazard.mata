...
`RM' _logl_loghazard(`TR' M, `RR' b, `gml' gml, `RM' G, `RM' H)
{	
	// Setup //
...
// 			}
			...
			chq2 = uhtred_util_tb_qs(M,b,gml,Nobs2)
			...
			logl[index2,] 	= logl[index2,] :- chq :* expxzb1
		}
		else {
	...
	if (Nobs3) {
		...
		logl[index3,] = -exp(-exp(xtzb[index3,]) :* y[index3,1])
		...
		logl[index5,] = logl[index5,] :+ exp(-exp(xtzb[index5,]) :* y[index5,tind])
		//logL
	...
	if (Nobs1) {
		if (hasxb) G[index1,sindex1] = X[index1,]
		if (hastb) G[index1,sindex2] = XT[index1,]
	}  
	...
	if (Nobs2) {
		if (hastb) {
			...
			Gtemp = exp(xtzb[index2,]) :* y[index2,1]
			G[index2,sindex1] = G[index2,sindex1] :- 
				Gtemp :* X[index2,]
			
			if (haslt) {
				Gtemp0 = exp(xtzb[index4,]) :* y[index4,3]
	...
	// note; no contrib's from d/d2 log hazard
	...
	if (Nobs2) {
		if (hastb) {
			...
			Hxbtb[index2,] 	= - chdxdt :* expxzb1
			Htb2[index2,] 	= - chdt2 :* expxzb1

		}
		else {
			Hxb2[index2,] = -Gtemp
			if (haslt) Hxb2[index4,] = Hxb2[index4,] :+ Gtemp0
		}
	}
